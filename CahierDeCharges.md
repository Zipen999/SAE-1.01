...
```fModifier``` Cette fonction permet de modifier toutes les valeurs d'un article, on chercher la valeur et si elle est présente on écrase les valeurs actuelle par ceux que l'utilisateur saisi avec vérfication, on enregistre dans le fichier __nom.txt__.
